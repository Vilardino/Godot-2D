...
@onready var attack_collision:CollisionShape2D = get_node(attack_path)
...
func verify_position(direction: Vector2) -> void:
	if direction.x>0:
		...
		player.wall_ray.target_position = Vector2(13.5,0)
	elif direction.x<0:
		flip_h = true
		suffix = "_left"
		player.direction =1
		position = Vector2(-2,0)
		player.wall_ray.target_position = Vector2(-15.5,0)
